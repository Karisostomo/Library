...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['books'])) {
    ...
    try {
        ...
        foreach ($books as $index => $book) {
            ...
            // Debugging: Print the values being inserted (comment out for production)
            // echo '<pre>';
            // echo "Inserting: $category, $name, $author, $isbn, $year";
            // echo '</pre>';

            $result = $stmt->execute([$category, $name, $author, $isbn, $year]);
            if (!$result) {
                // Debugging: Print SQL errors (comment out for production)
                // echo '<pre>';
